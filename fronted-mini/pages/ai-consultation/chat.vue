	...
	<view :class="['container', rootFontClass]">
		...
		<view class="custom-navbar">
			<view class="navbar-content">
				...
				<view class="navbar-title">AI医生</view>
				...
				<view class="message-item message-ai" v-if="messages.length === 0">
					...
					<view class="message-content">
						<view class="message-bubble ai-bubble">
							<text class="message-text">您好！我是AI医生，有什么健康问题可以咨询我。我会为您提供专业的健康建议。</text>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			// 加载历史对话
			loadChatHistory() {
				const historyKey = `aiChatHistory_${this.userId}`
			...
			// 发送消息（调用主页面对话接口 /home-chat/ask，与智能体交互）
			sendMessage() {
				if (!this.canSend) {
					return
				}
				...
				uni.request({
					...
					fail: (err) => {
						...
						const errorMessage = {
							role: 'ai',
							content: err.errMsg && err.errMsg.includes('timeout') ? '请求超时，请稍后再试。' : '网络错误，请检查网络连接后重试。',
							timestamp: new Date()
						}
						this.messages.push(errorMessage)
						this.saveChatHistory()
